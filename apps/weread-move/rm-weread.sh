...
cleanup_stale_instances() {
    app_path="${1}"
    ps ww | awk -v app="$app_path" -v self="$$" '
        $0 ~ app && $0 !~ /awk/ {
            pid = $1
...
if [ "${RM_WEREAD_LEGACY_UI:-0}" = "1" ]; then
    cleanup_stale_instances "${APP_DIR}/app.lua"
    cd "${KO_DIR}"
    exec ./luajit "${APP_DIR}/app.lua" >>"${LOG_DIR}/rm-weread.log" 2>&1
fi
...
cleanup_stale_instances "${APP_DIR}/app.lua"
cleanup_stale_instances "${APP_DIR}/native_app.lua"
cd "${KO_DIR}"
