...
assert(grid.includes('acceptedDevices: PointerDevice.TouchScreen'), 'shelf swipe must respond to touch rather than stylus input');
...
assert(grid.includes('dx < 0') && grid.includes('root.goShelfPage(1)'), 'left swipe must show the next shelf page');
assert(grid.includes('dx > 0') && grid.includes('root.goShelfPage(-1)'), 'right swipe must show the previous shelf page');
