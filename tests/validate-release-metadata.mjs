...
assert(version === '2.0.0', 'source milestone version must be explicit and stable');
...
assert(cmake.includes('project(rm_weread_qt VERSION 2.0.0'), 'CMake project version must match VERSION');
...
assert(changelog.includes(`## [${version}]`) && releaseNotes.includes('Source-only 2.0.0 milestone'), 'release notes must match the staged version and boundary');
