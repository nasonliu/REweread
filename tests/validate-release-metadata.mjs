...
assert(version === '1.0.0-rc.1', 'release candidate version must be explicit and stable');
assert(manifest.version === version, 'release manifest must match VERSION');
assert(manifest.releaseType === 'source-only', 'release candidate must remain source-only');
assert(manifest.commercialUse === false && manifest.officiallyAuthorized === false, 'manifest must preserve legal boundaries');
assert(cmake.includes('project(rm_weread_qt VERSION 1.0.0'), 'CMake project version must match the 1.0 line');
...
assert(changelog.includes(`## [${version}]`) && releaseNotes.includes('Source-only release candidate'), 'release notes must match the staged version and boundary');
